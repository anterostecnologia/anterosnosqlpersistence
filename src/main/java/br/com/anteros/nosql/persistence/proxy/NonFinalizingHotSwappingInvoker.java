...
import br.com.anteros.nosql.persistence.metadata.annotations.IdGetter;
...
class NonFinalizingHotSwappingInvoker<T> extends HotSwappingInvoker<T> {
	...
	NonFinalizingHotSwappingInvoker(final Class<?>[] types, final ProxyFactory proxyFactory,
			final ObjectReference<Object> delegateReference, final DelegationMode delegationMode) {
	...
	public Object invoke(final Object proxy, final Method method, final Object[] args) throws Throwable {
		...
		/*
		 * If the method being invoked is annotated with @IdGetter and the delegate
		 * reference is an EntityObjectReference, return the id of the
		 * EntityObjectReference's key. This allows us to return the referenced entity's
		 * id without fetching the entity from the datastore.
		 */
