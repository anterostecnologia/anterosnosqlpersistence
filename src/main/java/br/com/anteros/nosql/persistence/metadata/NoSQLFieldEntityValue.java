...
public class NoSQLFieldEntityValue implements Comparable<NoSQLFieldEntityValue> {
	...
	public NoSQLFieldEntityValue(String fieldName, Object value, Object source) {
		this.name = fieldName;
		this.value = value;
		this.source = source;
	}

	public int compareTo(NoSQLFieldEntityValue target) {
		if (target == null)
			return -1;
		if (value instanceof NoSQLFieldEntityValue[]) {
			if (((NoSQLFieldEntityValue[]) value).length != ((NoSQLFieldEntityValue[]) target.getValue()).length) {
				return -1;
			}
			for (int i = 0; i < ((NoSQLFieldEntityValue[]) value).length; i++) {
				NoSQLFieldEntityValue sourceField = ((NoSQLFieldEntityValue[]) value)[i];
				boolean found = false;
				for (int k = 0; k < ((NoSQLFieldEntityValue[]) target.getValue()).length; k++) {
					NoSQLFieldEntityValue targetField = ((NoSQLFieldEntityValue[]) target.getValue())[k];
	...
	public static NoSQLFieldEntityValue[] getEntityValuesDifference(NoSQLFieldEntityValue[] source, NoSQLFieldEntityValue[] target) {
		List<NoSQLFieldEntityValue> result = new ArrayList<NoSQLFieldEntityValue>();
		for (NoSQLFieldEntityValue entitySource : source) {
			boolean found = false;
			for (NoSQLFieldEntityValue entityTarget : target) {
		...
		return result.toArray(new NoSQLFieldEntityValue[] {});
	}

	@Override
	public String toString() {
		StringBuilder result = new StringBuilder();
		result.append(name).append(" tipo->").append((value instanceof NoSQLFieldEntityValue[] ? "COLLECTION" : "SIMPLE")).append(" valor->");
		if (value instanceof NoSQLFieldEntityValue[]) {
			result.append("{");
			boolean appendComma = false;
			for (NoSQLFieldEntityValue v : (NoSQLFieldEntityValue[]) value) {
				if (appendComma)
					result.append(", ");
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		NoSQLFieldEntityValue other = (NoSQLFieldEntityValue) obj;
		if (name == null) {
			if (other.name != null)
				return false;
